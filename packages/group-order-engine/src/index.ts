export type SupportedCurrency = 'EGP' | 'USD' | 'EUR' | 'GBP' | 'SAR' | 'AED';
...
export const calculateBasisPointCharge = (
  amountMinor: number,
  basisPoints: number,
): number => {
...
const buildItemAttribution = (receipts: ParticipantReceipt[]): ItemAttribution[] => {
  ...
  return [...items.values()]
    .map((item) => ({
      ...item,
      orderedBy: item.orderedBy.toSorted((left, right) =>
...
export const calculateGroupOrderReceipt = (input: {
  ...
  const receiptTotal = participantReceipts.reduce(
    (sum, receipt) => sum + receipt.totalMinor,
    0,
  );

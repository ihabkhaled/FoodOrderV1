...
import { buildRepeatedOrderDraft, createOrder, transitionOrder } from '../helpers/order.helper';
import type { Bucket, BucketMember, Order, OrderStatus, SessionUser } from '../types/domain.types';
...
const readDatabase = (): LocalOrderDatabase => {
  const raw = readWebStorage(DATABASE_KEY);
...
const writeDatabase = (database: LocalOrderDatabase): void => {
  writeWebStorage(DATABASE_KEY, JSON.stringify(database));
};

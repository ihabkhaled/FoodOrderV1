...
const lifecycleActions = (
  ...
  if (status === ORDER_SESSION_STATUS.collecting) {
    return [
      ...
      {
        status: ORDER_SESSION_STATUS.cancelled,
    ...
    return [
      {
        ...
        labelKey: 'reopenOrder',
        danger: false,
      },
      {
        status: ORDER_SESSION_STATUS.submitted,
        labelKey: 'submitOrder',
        danger: false,
      },
      {
        status: ORDER_SESSION_STATUS.cancelled,
  ...
  if (status === ORDER_SESSION_STATUS.submitted) {
    return [
      {
        status: ORDER_SESSION_STATUS.confirmed,
        labelKey: 'confirmOrder',
        danger: false,
      },
      {
        status: ORDER_SESSION_STATUS.cancelled,
  ...
  if (status === ORDER_SESSION_STATUS.confirmed) {
    return [
      {
        status: ORDER_SESSION_STATUS.delivered,
  ...
  if (status === ORDER_SESSION_STATUS.delivered) {
    return [
      {
        status: ORDER_SESSION_STATUS.settling,
        labelKey: 'startSettlement',
        danger: false,
      },
    ];
  }
...
export interface SessionCommandCenterViewModel {

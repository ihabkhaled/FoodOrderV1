...
export function OrdersPage() {
  ...
  const remove = async () => { if (!user || !deleting) return; try { await dataService.deleteOrder(user.id, deleting.id); setOrders((current) => current?.filter((order) => order.id !== deleting.id) ?? []); showToast(t('orderDeleted'), 'success'); } catch (reason) { showToast(reason instanceof Error ? reason.message : t('tryAgain'), 'error'); } finally { setDeleting(null); } };

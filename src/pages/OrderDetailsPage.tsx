...
export function OrderDetailsPage() {
  ...
  useEffect(() => { if (!user || !orderId) return; void dataService.getOrder(user.id, orderId).then((value) => { if (!value) throw new Error(t('tryAgain')); setOrder(value); }).catch((reason: unknown) => { setError(reason instanceof Error ? reason.message : t('tryAgain')); }).finally(() => { setLoading(false); }); }, [orderId, user, t]);
  const transition = async (status: OrderStatus) => { if (!user || !order) return; try { const saved = await dataService.updateOrderStatus(user.id, order.id, status); setOrder(saved); showToast(t('orderUpdated'), 'success'); } catch (reason) { showToast(reason instanceof Error ? reason.message : t('tryAgain'), 'error'); } };
  const repeat = async () => { if (!user || !order) return; try { const created = await dataService.createOrder(user.id, { bucketId: order.bucketId, bucketTitle: order.bucketTitle, currency: order.currency, lines: order.lines.map(({ id, bucketItemId, name, quantity, unitPrice }) => ({ id, bucketItemId, name, quantity, unitPrice })), notes: order.notes, status: 'draft' }); showToast(t('draftFromOrder'), 'success'); await navigate(`/orders/${created.id}`); } catch (reason) { showToast(reason instanceof Error ? reason.message : t('tryAgain'), 'error'); } };

...
export function DashboardPage() {
  ...
  const load = useCallback(async () => {
    ...
    catch (error_) { setError(error_ instanceof Error ? error_.message : 'Unable to load dashboard.'); }
  }, [user]);
  ...
  return <div className="page stack-lg">
    ...
    <section className="section-card"><div className="section-heading"><div><p className="eyebrow">{t('recentOrders')}</p><h2>{t('myOrders')}</h2></div><Link to="/orders">{t('orders')}</Link></div>
      {summary.recentOrders.length > 0 ? <div className="list">{summary.recentOrders.map((order) => <Link to={`/orders/${order.id}`} className="list-row" key={order.id}><div><strong>{order.bucketTitle}</strong><span>{formatDateTime(order.createdAt, locale)}</span></div><div className="row-end"><StatusBadge status={order.status} /><strong>{formatMoney(order.total, order.currency, locale)}</strong></div></Link>)}</div> : <p className="muted">{t('emptyOrders')}</p>}
    </section>
  </div>;
}

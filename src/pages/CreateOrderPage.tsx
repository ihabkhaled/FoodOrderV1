...
export function CreateOrderPage() {
  ...
  useEffect(() => { if (!user || !bucketId) return; void dataService.getBucket(user, bucketId).then((value) => { if (!value) throw new Error('Bucket was not found.'); setBucket(value); }).catch((error_: unknown) => { setError(error_ instanceof Error ? error_.message : 'Unable to load bucket.'); }).finally(() => { setLoading(false); }); }, [bucketId, user]);
  ...
  const submit = async (status: 'draft' | 'placed') => { if (!user || !bucket) return; if (selectedLines.length === 0) { setError(t('noItemsSelected')); return; } try { setBusy(true); setError(''); const order = await dataService.createOrder(user.id, { bucketId: bucket.id, bucketTitle: bucket.title, currency: bucket.currency, lines: selectedLines, notes, status }); showToast(status === 'placed' ? t('orderPlaced') : t('draftSaved'), 'success'); await navigate(`/orders/${order.id}`); } catch (error_) { setError(error_ instanceof Error ? error_.message : t('tryAgain')); } finally { setBusy(false); } };

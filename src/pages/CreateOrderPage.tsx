...
export function CreateOrderPage() {
  ...
  useEffect(() => { if (!user || !bucketId) return; void dataService.getBucket(user.id, bucketId).then((value) => { if (!value) throw new Error('Bucket was not found.'); setBucket(value); }).catch((reason: unknown) => setError(reason instanceof Error ? reason.message : 'Unable to load bucket.')).finally(() => setLoading(false)); }, [bucketId, user]);
  ...
  const submit = async (status: 'draft' | 'placed') => { if (!user || !bucket) return; if (!selectedLines.length) return setError(t('noItemsSelected')); try { setBusy(true); setError(''); const order = await dataService.createOrder(user.id, { bucketId: bucket.id, bucketTitle: bucket.title, currency: bucket.currency, lines: selectedLines, notes, status }); showToast(status === 'placed' ? 'Order placed.' : 'Draft saved.', 'success'); navigate(`/orders/${order.id}`); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Unable to create order.'); } finally { setBusy(false); } };
  ...
  return <div className="page narrow"><Link className="back-link" to="/buckets"><ArrowLeft />{t('back')}</Link><div className="page-heading"><div><p className="eyebrow">{t('orderNow')}</p><h1>{bucket.title}</h1><p>{bucket.description}</p></div><div className="total-block"><span>{t('total')}</span><strong>{formatMoney(total, bucket.currency, locale)}</strong></div></div>
    <form className="stack-lg"><section className="section-card order-picker">{bucket.items.filter((item) => item.active).map((item) => <article className="order-line" key={item.id}><div><h3>{item.name}</h3><span>{item.category || 'Item'} · {formatMoney(item.unitPrice, bucket.currency, locale)}</span></div><div className="quantity-control"><button type="button" className="icon-button" onClick={() => adjust(item.id, -1)} aria-label={`Decrease ${item.name}`}><Minus /></button><output>{quantities[item.id] ?? 0}</output><button type="button" className="icon-button" onClick={() => adjust(item.id, 1)} aria-label={`Increase ${item.name}`}><Plus /></button></div></article>)}</section><section className="section-card"><label>{t('notes')}<textarea rows={4} maxLength={500} value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Special instructions or group name" /></label></section>{error ? <p className="form-error">{error}</p> : null}<div className="sticky-actions"><button type="button" className="button secondary" disabled={busy || !selectedLines.length} onClick={() => void submit('draft')}>{t('saveDraft')}</button><button type="button" className="button" disabled={busy || !selectedLines.length} onClick={() => void submit('placed')}><ShoppingCart />{busy ? t('loading') : t('placeOrder')}</button></div></form>
  </div>;
}

...
import type { Bucket, BucketMember, SessionUser } from '@/types/domain';
import type {
  BucketAccessGrant,
  FriendGroup,
  FriendRequest,
...
const readSocialDatabase = (): LocalSocialDatabase => {
  ...
  try {
    return { ...emptySocialDatabase(), ...(JSON.parse(raw) as LocalSocialDatabase) };
  } catch {
...
const activeGroupRecipients = (
  ...
  return group.members
    ...
    .map(({ userId, displayName, email }) => ({ userId, displayName, email }));
};
...
const updateBucketMember = (
  ...
  const existing = members[index] as (BucketMember & { accessSources?: string[] }) | undefined;
  const timestamp = new Date().toISOString();
  const saved: BucketMember & { accessSources: string[] } = {
    userId: recipient.userId,
    displayName: recipient.displayName,
    email: recipient.email,
    role: existing?.role === 'editor' ? 'editor' : role,
    status: 'active',
    canCreateCustomItems:
      existing?.canCreateCustomItems ?? role === 'editor',
    canSetCustomItemPrice:
      existing?.canSetCustomItemPrice ?? role === 'editor',
    invitedBy: bucket.ownerId,
    joinedAt: existing?.joinedAt ?? timestamp,
    updatedAt: timestamp,
    accessSources: [...new Set([...(existing?.accessSources ?? []), sourceId])],
  };
...
const materializeGrant = (
  ...
  const owned = database.buckets[actor.userId] ?? [];
  const index = owned.findIndex((bucket) => bucket.id === grant.bucketId);
  const bucket = owned[index];
  if (!bucket) throw new Error('Only the bucket owner may share it.');
  const shared: Bucket = {
    ...bucket,
    visibility: 'shared',
  ...
  owned[index] = shared;
  database.buckets[actor.userId] = owned;
  for (const recipient of recipients) {
    if (recipient.userId !== actor.userId) {
...
export class LocalSocialService implements SocialService {
  ...
  getOverview(): Promise<SocialOverview> {
    ...
    return Promise.resolve({
      ...
      incomingRequests: database.requests.filter(
        (request) =>
          request.recipient.userId === actor.userId && request.status === 'pending',
      ),
      outgoingRequests: database.requests.filter(
        (request) =>
          request.sender.userId === actor.userId && request.status === 'pending',
      ),
  ...
  async sendFriendRequest(email: string): Promise<FriendRequest> {
    ...
    const database = readSocialDatabase();
    if ((database.friends[sender.userId] ?? []).some((friend) => friend.userId === recipient.userId)) {
  ...
  inviteFriendToGroup(groupId: string, friendId: string): Promise<void> {
    ...
    group.members = [...group.members.filter((item) => item.userId !== friendId), member];
    group.updatedAt = timestamp;
    database.invitations = [
      ...database.invitations.filter(
        (invitation) =>
          invitation.groupId !== groupId || invitation.recipient.userId !== friendId,
      ),
      {
        groupId,
  ...
  respondGroupInvitation(
    ...
    const database = readSocialDatabase();
    const group = database.groups.find((candidate) => candidate.id === groupId);
    const member = group?.members.find((candidate) => candidate.userId === recipient.userId);
  ...
  shareBucketWithGroup(
    ...
    database.grants = [
      ...database.grants.filter(
        (candidate) => candidate.bucketId !== bucketId || candidate.id !== grant.id,
      ),
      grant,
    ];
    writeSocialDatabase(database);

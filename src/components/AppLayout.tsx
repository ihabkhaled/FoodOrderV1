...
const THEME_LABEL: Record<Theme, MessageKey> = { system: 'themeSystem', light: 'themeLight', dark: 'themeDark' };

export function AppLayout() {
  const { t, user, logout, online, storageMode, toast, locale, theme, setDeviceLocale, setDeviceTheme } = useApp();
  const location = useLocation();
  const [collapsed, setCollapsed] = useState(false);

  useEffect(() => {
    void loadSidebarCollapsed().then(setCollapsed).catch(() => { setCollapsed(false); });
  }, []);
  ...
  const connection = (
    <span className={`connection ${online ? 'online' : 'offline'}`}>
      ...
      <span className="label-collapsible">{online ? t('online') : t('offline')}</span>
    </span>
  );
  const localeButton = (
    <button className="icon-button" onClick={() => void setDeviceLocale(otherLocale)}
      title={otherLocale === 'ar' ? t('switchToArabic') : t('switchToEnglish')}
      aria-label={otherLocale === 'ar' ? t('switchToArabic') : t('switchToEnglish')}>
      <Languages />
    </button>
  );
  const themeButton = (
    <button className="icon-button" onClick={() => void setDeviceTheme(nextTheme(theme))}
      title={t(THEME_LABEL[theme])} aria-label={t(THEME_LABEL[theme])}>
      <ThemeIcon />
    </button>
  );
  const logoutButton = (
    ...
    <div className={`app-shell${collapsed ? ' collapsed' : ''}`}>
      ...
      <aside className="sidebar">
        <div className="sidebar-head">
          <NavLink to="/" className="brand"><span className="brand-mark">FO</span><span className="label-collapsible">{t('appName')}</span></NavLink>
          <button className="icon-button collapse-toggle" onClick={toggleCollapsed}
        ...
        <nav className="sidebar-nav" aria-label={t('primaryNavigation')}>
          {NAV_ITEMS.map(({ to, icon: Icon, key }) => (
            <NavLink key={to} to={to} end={to === '/'} title={t(key)}
              ...
              <Icon /><span className="label-collapsible">{t(key)}</span>
            </NavLink>
          ))}
        </nav>
        <div className="sidebar-footer">
          <div className="sidebar-controls">{localeButton}{themeButton}</div>
          {connection}
          <span className="mode-pill"><span className="label-collapsible">{storageMode === 'firebase' ? t('firebaseMode') : t('localMode')}</span></span>
          <div className="sidebar-user">
            <span className="user-name label-collapsible">{user?.displayName}</span>
            {logoutButton}
          </div>
        </div>
      </aside>

      {/* Mobile: slim top bar with the same instant controls */}
      <header className="topbar">
        <NavLink to="/" className="brand"><span className="brand-mark">FO</span><span>{t('appName')}</span></NavLink>
        <div className="topbar-meta">
          {localeButton}{themeButton}{connection}{logoutButton}
        </div>
      </header>

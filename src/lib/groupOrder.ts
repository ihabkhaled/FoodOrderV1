...
export interface GroupOrderChargePolicy {
...
export interface ParticipantItemInput {
  itemId: string;
  itemName: string;
  quantity: number;
...
export interface GroupOrderParticipantInput {
  userId: string;
  displayName: string;
  items: ParticipantItemInput[];
}

export interface ParticipantReceiptLine extends ParticipantItemInput {
...
export interface GroupOrderReceipt {
  ...
  participantReceipts: ParticipantReceipt[];
...
export const calculateBasisPointCharge = (amountMinor: number, basisPoints: number): number => {
...
export const allocateMinorUnits = (
  totalMinor: number,
  ...
  for (const entry of entries) {
    if (!entry.id.trim()) throw new Error('Allocation recipient identifiers are required.');
    if (identifiers.has(entry.id)) throw new Error('Allocation recipient identifiers must be unique.');
  ...
  return Object.fromEntries(
    calculations.map((entry) => [entry.id, toSafeNumber(entry.allocated, 'Allocated share')]),
  );
};

const calculateLineTotal = (item: ParticipantItemInput): number => {
  if (!item.itemId.trim() || !item.itemName.trim()) throw new Error('Receipt items require an identity and name.');
...
const allocationEntries = (
  receipts: Pick<ParticipantReceipt, 'userId' | 'itemSubtotalMinor'>[],
  strategy: ChargeAllocationStrategy,
): AllocationEntry[] =>
...
const buildItemAttribution = (receipts: ParticipantReceipt[]): ItemAttribution[] => {
  const items = new Map<string, ItemAttribution>();
...
export const calculateGroupOrderReceipt = (input: {
  ...
  policy: GroupOrderChargePolicy;
}): GroupOrderReceipt => {
  if (input.participants.length === 0) throw new Error('A group order requires at least one participant.');

  const participantIds = new Set<string>();
  const baseReceipts: ParticipantReceipt[] = input.participants.map((participant) => {
  ...
  const itemSubtotalMinor = baseReceipts.reduce(
    (sum, receipt) => sum + receipt.itemSubtotalMinor,
    0,
  );
  ...
  const serviceMinor = calculateBasisPointCharge(
    itemSubtotalMinor,
    input.policy.serviceBasisPoints,
  );
  const vatShares = allocateMinorUnits(
    vatMinor,
    allocationEntries(baseReceipts, input.policy.vatAllocation),
  );
  const serviceShares = allocateMinorUnits(
    serviceMinor,
    allocationEntries(baseReceipts, input.policy.serviceAllocation),
  );
  const deliveryShares = allocateMinorUnits(
    input.policy.deliveryMinor,
    allocationEntries(baseReceipts, input.policy.deliveryAllocation),
  );

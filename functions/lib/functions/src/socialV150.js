...
import { queueNotification, writeNotifications, } from './notificationCore.js';
import { canInviteGroupMember, optionalText, removeAccessSource, requiredText, strongestRoleFromGrants, } from './socialDomain.js';
...
const ownedGroup = async (groupId, ownerId) => {
    ...
    if (group.ownerId !== ownerId) {
        throw new HttpsError('permission-denied', 'Only the group owner may perform this action.');
    }
    return { reference, group };
};
const revokeBucketMembership = async (bucketId, userId) => {
    const timestamp = new Date().toISOString();
    const batch = db().batch();
    batch.set(db().collection('buckets').doc(bucketId).collection('members').doc(userId), {
        status: 'revoked',
        accessSources: [],
...
const detachGroupAccess = async (bucketId, groupId, userId) => {
    ...
    const accessSources = removeAccessSource(member.accessSources, `${GROUP_SOURCE_PREFIX}${groupId}`);
    if (accessSources.length === 0) {
        await revokeBucketMembership(bucketId, userId);
        return;
    }
    const grantReferences = accessSources.map((sourceId) => db()
        .collection('buckets')
        .doc(bucketId)
    ...
    batch.set(userSubcollection(userId, 'bucketMemberships').doc(bucketId), { role, accessSources, updatedAt: timestamp }, { merge: true });
    await batch.commit();
};
const groupGrants = async (groupReference) => {
    const snapshot = await groupReference.collection('bucketGrants').get();
    return snapshot.docs.map((document) => document.data());
};
const detachAllGroupAccess = async (groupReference, groupId, userId) => {
    const grants = await groupGrants(groupReference);
    await Promise.all(grants.map((grant) => detachGroupAccess(grant.bucketId, groupId, userId)));
};
export const unfriendV150 = onCall({ region: REGION }, async (request) => {
    const actor = authUser(request.auth);
    const friendId = invalidArgument(() => requiredText(dataOf(request.data).friendId, 'Friend ID', 160));
    if (friendId === actor.userId) {
        throw new HttpsError('failed-precondition', 'You cannot unfriend yourself.');
    }
    const actorFriendReference = userSubcollection(actor.userId, 'friends').doc(friendId);
...
export const inviteFriendToGroupV150 = onCall({ region: REGION }, async (request) => {
    ...
    if (!canInviteGroupMember(existingStatus)) {
        throw new HttpsError('already-exists', 'This friend is already in or invited to the group.');
    }
    const invitedFriend = friendSnapshot.data();
    const timestamp = new Date().toISOString();
    const member = {
        ...invitedFriend,
        status: 'pending',
...
export const updateFriendGroupV150 = onCall({ region: REGION }, async (request) => {
    const owner = authUser(request.auth);
    const input = dataOf(request.data);
    const groupId = invalidArgument(() => requiredText(input.groupId, 'Group ID', 160));
    ...
    for (const document of membersSnapshot.docs) {
        ...
        if (member.status === 'active') {
            batch.set(userSubcollection(member.userId, 'groupMemberships').doc(groupId), { groupName: name, updatedAt: timestamp }, { merge: true });
            if (member.userId !== owner.userId) {
                activeRecipients.push(member.userId);
            }
        }
        if (member.userId !== owner.userId) {
            batch.set(userSubcollection(member.userId, 'groupInvitations').doc(groupId), { groupName: name }, { merge: true });
        }
    }
    for (const document of grantsSnapshot.docs) {
        ...
        batch.set(db()
            ...
            .doc(grant.bucketId)
            .collection('accessGrants')
            .doc(grant.id), { subjectName: name }, { merge: true });
    }
    await batch.commit();
...
export const removeFriendGroupMemberV150 = onCall({ region: REGION }, async (request) => {
    const owner = authUser(request.auth);
    const input = dataOf(request.data);
    const groupId = invalidArgument(() => requiredText(input.groupId, 'Group ID', 160));
    const memberId = invalidArgument(() => requiredText(input.memberId, 'Member ID', 160));
    const { reference, group } = await ownedGroup(groupId, owner.userId);
    ...
    if (member.status !== 'active' && member.status !== 'pending') {
        throw new HttpsError('failed-precondition', 'Only active or pending members may be removed.');
    }
    if (member.status === 'active') {
    ...
    const timestamp = new Date().toISOString();
    const batch = db().batch();
    batch.set(memberReference, { status: 'removed', respondedAt: timestamp }, { merge: true });
    batch.set(userSubcollection(memberId, 'groupInvitations').doc(groupId), { status: 'removed', respondedAt: timestamp }, { merge: true });
...
export const leaveFriendGroupV150 = onCall({ region: REGION }, async (request) => {
    const actor = authUser(request.auth);
    const groupId = invalidArgument(() => requiredText(dataOf(request.data).groupId, 'Group ID', 160));
    const reference = db().collection('friendGroups').doc(groupId);
    ...
    if (group.ownerId === actor.userId) {
        throw new HttpsError('failed-precondition', 'The group owner must delete the group instead.');
    }
    ...
    const timestamp = new Date().toISOString();
    const batch = db().batch();
    batch.set(memberSnapshot.ref, { status: 'left', respondedAt: timestamp }, { merge: true });
    batch.set(userSubcollection(actor.userId, 'groupInvitations').doc(groupId), { status: 'left', respondedAt: timestamp }, { merge: true });
...
export const deleteFriendGroupV150 = onCall({ region: REGION }, async (request) => {
    const owner = authUser(request.auth);
    const groupId = invalidArgument(() => requiredText(dataOf(request.data).groupId, 'Group ID', 160));
    const { reference, group } = await ownedGroup(groupId, owner.userId);
    const [membersSnapshot, grantsSnapshot] = await Promise.all([
    ...
    const activeRecipients = membersSnapshot.docs
        ...
        .filter((member) => member.userId !== owner.userId && member.status === 'active');
    await Promise.all(activeRecipients.flatMap((member) => grantsSnapshot.docs.map((document) => detachGroupAccess(document.data().bucketId, groupId, member.userId))));
    const batch = db().batch();
    for (const document of membersSnapshot.docs) {
        const member = document.data();
    ...
    await batch.commit();
    await writeNotifications(activeRecipients.map((member) => member.userId), {
        kind: 'group_deleted',
        title: 'Group deleted',

...
import { assertGroupOrderDocument, buildGroupOrderTransitionPatch, repeatGroupOrderDocument, } from './orderLifecycle.js';
if (getApps().length === 0)
    initializeApp();
...
const throwCallableError = (operation, error, context) => {
    ...
    throw new HttpsError(error instanceof Error && error.message.startsWith('Order cannot move')
        ? 'failed-precondition'
        : 'internal', error instanceof Error && error.message.startsWith('Order cannot move')
        ? error.message
        : 'The cloud action could not be completed. Please try again.');
};
const assertManagerAccess = async (transaction, bucketReference, ownerId, actorId) => {
    if (actorId === ownerId)
        return;
    ...
    if (member?.status !== 'active' || member.role !== 'editor') {
        throw new HttpsError('permission-denied', 'Only the bucket owner or an active editor may change this group order.');
    }
};
const loadManagedOrder = async (transaction, actorId, orderId) => {
    ...
    const actorOrder = actorSnapshot.data();
...
const transitionGroupOrderHandler = async (request) => {
    ...
    try {
        return await firestore.runTransaction(async (transaction) => {
            const context = await loadManagedOrder(transaction, actor.userId, orderId);
            const timestamp = new Date().toISOString();
            const patch = buildGroupOrderTransitionPatch(context.canonicalOrder.status, status, timestamp);
            ...
            if (typeof placedById === 'string' && placedById)
                recipientIds.add(placedById);
            const references = [...recipientIds].map((userId) => firestore.collection('users').doc(userId).collection('orders').doc(orderId));
...
const repeatGroupOrderHandler = async (request) => {
    const actor = requireActor(request);
    const data = callableData(request.data);
    const orderId = requiredString(data.orderId, 'Order ID', 160);
    try {
        return await firestore.runTransaction(async (transaction) => {
            ...
            const reference = firestore
                ...
                .collection('orders')
                .doc();
            const repeated = repeatGroupOrderDocument(context.actorOrder, actor.userId, reference.id, new Date().toISOString());
...
export const transitionGroupOrderV133 = onCall({ region: REGION }, transitionGroupOrderHandler);
export const repeatGroupOrderV133 = onCall({ region: REGION }, repeatGroupOrderHandler);

...
const SHARE_ROLE_VALUES = [
    'editor',
    'contributor',
    'viewer',
];
export const isShareRole = (value) => SHARE_ROLE_VALUES.includes(value);
export const shareRole = (value) => {
    if (isShareRole(value))
        return value;
...
export const strongestRoleFromGrants = (roles) => {
    const validRoles = roles.filter((role) => isShareRole(role));
...
export const mergeAccessSources = (current, sourceId) => {
    const existing = Array.isArray(current)
        ...
        : [];
    return [...new Set([...existing, sourceId])].toSorted((left, right) => left.localeCompare(right));
};
...
export const removeAccessSource = (current, sourceId) => {
    const existing = Array.isArray(current)
        ? current.filter((value) => typeof value === 'string')
        : [];
    return [...new Set(existing.filter((value) => value !== sourceId))].toSorted((left, right) => left.localeCompare(right));
};

...
import { effectiveMemberCustomItemPermissions, } from './memberPermissions.js';
    ...
    try {
        return await firestore.runTransaction(async (transaction) => {
            ...
            if (state === 'ordered' && bucket.lastOrderId) {
                const existing = await transaction.get(firestore
                    .collection('users')
                    .doc(ownerId)
                    .collection('orders')
            ...
            const { order, aggregate } = buildOrderSnapshot(bucket, contributions, ownerId, notes, orderId, timestamp);
...
export const addCustomBucketItem = onCall({ region: REGION }, async (request) => {
    ...
    return firestore.runTransaction(async (transaction) => {
        ...
        if (bucket.ownerId !== userId) {
            const memberSnapshot = await transaction.get(bucketReference.collection('members').doc(userId));
            if (!memberSnapshot.exists) {
                throw new HttpsError('permission-denied', 'Bucket membership is required.');
            }
            const member = memberSnapshot.data();
            const canContribute = member.status === 'active' &&

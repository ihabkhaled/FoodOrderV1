...
import {
  ...
  type MemberOrderPermissionRecord,
...
interface MemberRecord extends MemberOrderPermissionRecord {
  userId?: string;
  displayName?: string;
}
...
interface PlacedOrderRecord extends OrderRecord {
  placedById: string;
  placedByName: string;
}
...
const normalizeBucket = (
  ...
  if (!ownerId) {
    throw new HttpsError(
      'failed-precondition',
      'The bucket owner information is missing.',
    );
  }
  return {
    ...raw,
...
const actorOrder = (
  order: PlacedOrderRecord,
  actorId: string,
): PlacedOrderRecord => {
  if (actorId === order.userId) return order;
  const isParticipant = order.groupReceipt.participantReceipts.some(
  ...
  try {
    return await firestore.runTransaction(async (transaction) => {
      ...
      const bucket = normalizeBucket(
        bucketSnapshot.data() as BucketRecord,
        bucketId,
      );
...
const addCustomBucketItemHandler = async (
  request: CallableRequest<unknown>,
    ...
    return await firestore.runTransaction(async (transaction) => {
      ...
      if (bucket.ownerId !== actor.userId) {
        ...
        if (!member || member.status !== 'active') {
          throw new HttpsError(
            'permission-denied',
            'Active bucket membership is required.',
          );
        }
        const canContribute =
          member.role === 'editor' || member.role === 'contributor';
...
const approveCustomBucketItemHandler = async (
  request: CallableRequest<unknown>,
): Promise<BucketItemRecord> => {
  const actor = requireActor(request);
  const data = callableData(request.data);
...
export const finalizeGroupOrderV132 = onCall(
  { region: REGION },
  finalizeGroupOrderHandler,
);
export const addCustomBucketItemV132 = onCall(
  { region: REGION },
  addCustomBucketItemHandler,
);
export const approveCustomBucketItemV132 = onCall(
  { region: REGION },
  approveCustomBucketItemHandler,
);

export const finalizeGroupOrder = onCall(
  { region: REGION },
  finalizeGroupOrderHandler,
);

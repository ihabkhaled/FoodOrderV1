...
const inviteLinkReference = (token: string) => db().collection('inviteLinks').doc(token);
...
const readUsableLink = async (rawToken: unknown): Promise<InviteLinkRecord> => {
  ...
  if (!token) throw new HttpsError('not-found', 'This invite link is not valid.');
  const snapshot = await inviteLinkReference(token).get();
  if (!snapshot.exists) throw new HttpsError('not-found', 'This invite link is not valid.');
  const link = snapshot.data() as InviteLinkRecord;
...
const assertBucketOwner = async (bucketId: string, userId: string): Promise<string> => {
  ...
  const bucket = snapshot.data() as { ownerId?: string; title?: string };
...
const assertGroupOwner = async (groupId: string, userId: string): Promise<string> => {
  const snapshot = await db().collection('friendGroups').doc(groupId).get();
  if (!snapshot.exists) throw new HttpsError('not-found', 'Group was not found.');
  const group = snapshot.data() as { ownerId?: string; name?: string };
...
export const createInviteLinkV1100 = onCall({ region: REGION }, async (request) => {
  ...
  const existing = await db()
    ...
    .where('revoked', '==', false)
    .get();
  const live = existing.docs.filter((document) =>
    isInviteLinkUsable(document.data() as InviteLinkRecord),
  );
  if (live.length >= inviteLinkLimits.perUserPerKind) {
    throw new HttpsError(
      'resource-exhausted',
      `You already have ${inviteLinkLimits.perUserPerKind} active ${kind} links. Revoke one first.`,
    );
  }

  const createdAt = new Date();
  const expiresAt = new Date(
    createdAt.getTime() + INVITE_LINK_EXPIRY_HOURS[kind] * 3_600_000,
  ).toISOString();
  const token = generateToken();
  const record: InviteLinkRecord = {
    token,
    kind,
...
const redeemBucket = async (link: InviteLinkRecord, actor: Actor) => {
  const bucketId = link.bucketId as string;
  ...
  if (memberSnapshot.exists && (memberSnapshot.data() as { status?: string }).status === 'active') {
    return { kind: 'bucket' as const, bucketId, alreadyGranted: true };
  }
  const activeCount = membersSnapshot.docs.filter(
    (document) => (document.data() as { status?: string }).status === 'active',
  ).length;
  if (activeCount >= inviteLinkLimits.maxBucketMembers) {
    throw new HttpsError(
      'resource-exhausted',
      `A bucket supports up to ${inviteLinkLimits.maxBucketMembers} members.`,
    );
  }
  ...
  await batch.commit();
  return { kind: 'bucket' as const, bucketId, alreadyGranted: false };
};

const redeemFriend = async (link: InviteLinkRecord, actor: Actor) => {
  ...
  if (existing.exists) {
    return { kind: 'friend' as const, friendUserId: creatorId, alreadyGranted: true };
  }

  const creatorProfile = await db().collection('publicProfiles').doc(creatorId).get();
  const creator = creatorProfile.exists
    ? (creatorProfile.data() as Record<string, unknown>)
    : {};
  const timestamp = new Date().toISOString();
  const batch = db().batch();
  ...
  batch.set(userSubcollection(actor.userId, 'friends').doc(creatorId), {
    userId: creatorId,
    displayName:
      typeof creator.displayName === 'string' ? creator.displayName : link.createdByName,
  ...
  await batch.commit();
  return { kind: 'friend' as const, friendUserId: creatorId, alreadyGranted: false };
};

const redeemGroup = async (link: InviteLinkRecord, actor: Actor) => {
  ...
  if (!groupSnapshot.exists) throw new HttpsError('not-found', 'Group was not found.');
  const memberReference = groupReference.collection('members').doc(actor.userId);
  const memberSnapshot = await memberReference.get();
  if (memberSnapshot.exists && (memberSnapshot.data() as { status?: string }).status === 'active') {
    return { kind: 'group' as const, groupId, alreadyGranted: true };
  }

  const group = groupSnapshot.data() as { name?: string; ownerId?: string };
  const timestamp = new Date().toISOString();
  const batch = db().batch();
  batch.set(memberReference, {
    userId: actor.userId,
  ...
  await batch.commit();
  return { kind: 'group' as const, groupId, alreadyGranted: false };
};
...
export const redeemInviteLinkV1100 = onCall({ region: REGION }, async (request) => {
  const actor = authUser(request.auth);
  const link = await readUsableLink(dataOf(request.data).token);
  if (link.kind === 'bucket') return redeemBucket(link, actor);
  if (link.kind === 'group') return redeemGroup(link, actor);
...
export const revokeInviteLinkV1100 = onCall({ region: REGION }, async (request) => {
  ...
  if (!snapshot.exists) throw new HttpsError('not-found', 'This invite link is not valid.');
  if ((snapshot.data() as InviteLinkRecord).createdBy !== actor.userId) {
...
export const listInviteLinksV1100 = onCall({ region: REGION }, async (request) => {
  ...
  return {
    links: snapshot.docs
      .map((document) => document.data() as InviteLinkRecord)
